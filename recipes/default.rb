...
bash 'install Node.js' do
  ...
  code <<-EOF
    ...
    ln -s #{::Dir.home(node['conda']['user'])}/node-v#{node['node']['version']}-linux-x64/bin/npm /usr/local/bin/npm
    ln -s #{::Dir.home(node['conda']['user'])}/node-v#{node['node']['version']}-linux-x64/bin/npx /usr/local/bin/npx
  EOF
end
